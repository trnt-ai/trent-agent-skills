<!-- BEGIN:trent-security-validation -->
<!-- Auto-generated by trent-mcp-setup. Manual edits will be overwritten on update. -->
## Security Validation (MANDATORY - READ FIRST)

**BLOCKING REQUIREMENT**: You MUST call `mcp__trent__appsec` BEFORE presenting ANY
implementation plan that involves multiple steps, code changes, infrastructure, or
API designs. This is NON-NEGOTIABLE. Do NOT present the plan to the user first.

This applies to:
- Implementation proposals you create in response to feature requests
- Architecture or design recommendations
- Infrastructure or configuration changes

**Required workflow — follow this exact order:**
1. Draft your plan internally (do NOT output it to the user yet)
2. Call `mcp__trent__appsec` with:
   - `content`: The full implementation plan or approach
   - `context`: Security context about this project (e.g., language, framework, architecture)
3. Present the plan AND the security findings together to the user
4. Only proceed with implementation after user approval

**If you receive Stop hook feedback containing "SECURITY REVIEW REMINDER":**
This means you presented a plan without calling AppSec first. You MUST immediately:
1. Call `mcp__trent__appsec` with the plan you just presented
2. Present the security findings to the user
3. Do NOT continue with any other work until the security review is complete

Skip security validation only for trivial single-line fixes or documentation-only changes.
<!-- END:trent-security-validation -->
